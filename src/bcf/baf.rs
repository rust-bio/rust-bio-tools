...
pub fn calculate_baf() -> Result<(), Box<Error>> {
    ...
    let mut writer = bcf::Writer::from_stdout(
        &header,
        false,
        false
    )?;
    ...
    for record in reader.records() {
        ...
        {
            let ref_depths = record.format(b"RO").integer()?.into_iter().map(
                |d| d.to_owned()
            ).collect_vec();
            ...
            for (sample_ref_depth, sample_alt_depth) in ref_depths.iter().zip(alt_depths.iter()) {
                if allele_lens[0] != 1 || sample_ref_depth[0].is_missing() {
                    ...
                    let total_depth = sample_ref_depth[0] +
                                      sample_alt_depth.into_iter().sum::<i32>();
                    bafs.extend(
                        allele_lens[1..].iter().zip(
                            sample_alt_depth.iter()
                        ).map(|(alen, d)| {
                                if *alen == 1 {
